            ...
            <table class="min-w-full bg-white border border-emerald-100 rounded-lg">
                <thead class="bg-emerald-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-emerald-700 uppercase tracking-wider border-b border-emerald-200">คณะ</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-emerald-700 uppercase tracking-wider border-b border-emerald-200">รายละเอียด</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-emerald-700 uppercase tracking-wider border-b border-emerald-200">มูลค่า</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-emerald-700 uppercase tracking-wider border-b border-emerald-200">แต้มความดี</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    <template x-for="(row, idx) in donations" :key="row.donation_id">
                        <tr class="hover:bg-emerald-50 transition duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="formatDate(row.donation_date)"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="row.member_name || '-' "></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="row.faculty_name || '-' "></td>
                            <td class="px-6 py-4 text-sm text-gray-900" x-text="row.donation_description || '-' "></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="row.donation_estimated_value ?? '-' "></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="row.donation_goodness_point ?? '-' "></td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm space-x-2">
                                ...
                                <button @click="confirmDelete(row.donation_id)" class="text-red-600 hover:text-red-900 hover:bg-red-100 p-2 rounded-full transition">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="bg-white shadow-sm rounded-lg p-6 w-[28rem] max-w-full">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select x-model="form.member_id" @change="autoFillFaculty()" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                        <option value="">-- ไม่ระบุ --</option>
                        <template x-for="m in members" :key="m.member_id">
                            <option :value="String(m.member_id)" x-text="m.member_name + ' (#' + m.member_id + ')'"> </option>
                        </template>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea x-model="form.donation_description" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="รายละเอียดการบริจาค"></textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">มูลค่าโดยประมาณ</label>
                        <input type="number" step="0.01" x-model="form.donation_estimated_value" class="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="0.00">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">แต้มความดี</label>
                        <input type="number" x-model="form.donation_goodness_point" class="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="0">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">เหตุผล/หมายเหตุ</label>
                    <input type="text" x-model="form.donation_reason" class="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="หมายเหตุ">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">วันที่</label>
                    <input type="date" x-model="form.donation_date" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                </div>
            </div>
...
function DonationsPage() {
    return {
        ...
        getTotalValue() {
            return this.donations.reduce((s, d) => s + Number(d.donation_estimated_value || 0), 0);
        },
        ...
        openEdit(row) {
            ...
            this.form = {
                ...
                donation_estimated_value: row.donation_estimated_value ?? '',
