<div class="flex-cols space-y-2">
    <div x-data="FacultyMajor()" x-init="fetchAllFaculty()" class=" w-full">
        <div class="flex justify-between items-center mb-4">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-2xl font-bold text-slate-900">คณะ/สาขา/หน่วยบริการ</h1>
            </div>
            <div class="flex items-center gap-2">
                <button @click="window.open('/report/faculties_branches', '_blank','noopener')"
                    class="group cursor-pointer flex items-center py-2 px-3 border-2 border-indigo-500 rounded-full hover:bg-indigo-100 space-x-1 w-fit transition-colors font-medium text-indigo-700">
                    <svg class="w-5 h-5 stroke-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z">
                        </path>
                    </svg>
                    <span class="font-medium text-sm">พิมพ์รายงานทั้งหมด</span>
                </button>
            </div>
        </div>
        <div class="bg-white rounded-md shadow p-6 overflow-hidden h-full">
            ...
            <div class="flex justify-between mb-4">
                <div class="">
                    <h1 class="text-2xl font-bold text-slate-900">รายการคณะ</h1>
                    <p class="text-slate-600 font-light text-sm">จัดการข้อมูลรายการคณะ</p>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button @click="window.open('/report/faculties', '_blank','noopener')"
            ...
            <div class="mt-4 border-t border-gray-100 py-4 flex-1 overflow-auto relative">
                            ...
                            <tr :class="editingFacultyId === faculty.faculty_id ? 'bg-amber-50' : 'hover:bg-gray-50'"
                                ...
                                <td class="py-2 px-2 whitespace-nowrap text-sm text-gray-900">
                                    <div x-show="editingFacultyId !== faculty.faculty_id">
                                        <span
                                            x-text="`${faculty.faculty_code || ' ? '} : ${faculty.faculty_name}`"></span>
                                    </div>
    ...
    <div x-data="Branch()" x-init="fetchAllBranch()" class="w-full">
        <div class="bg-white rounded-md shadow p-6 overflow-hidden h-full">

            <div class="flex justify-between mb-4">
                <div class="">
                    <h1 class="text-2xl font-bold text-slate-900">รายการหน่วยบริการ</h1>
                    <p class="text-slate-600 font-light text-sm">จัดการข้อมูลรายการหน่วยบริการ</p>
                </div>
                <div class="flex items-center gap-2">
                    <div @click="openCreateDialog" :class="DialogShow && 'bg-emerald-300'"
                        ...
                        <span class="font-medium text-sm ">เพิ่มหน่วยบริการ</span>
                    </div>
                    <button @click="window.open('/report/branches', '_blank','noopener')"
                        class="group cursor-pointer flex items-center py-2 px-3 border-2 border-blue-500 rounded-full hover:bg-blue-100 space-x-1 w-fit transition-colors font-medium text-blue-700">
                        <svg class="w-5 h-5 stroke-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <dialog x-show="DialogShow" x-ref="createDialog" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="bg-white p-6 rounded-lg shadow-xl w-80">
                    <div class="flex justify-between ">
                        <h3 class="font-bold text-lg mb-3">เพิ่มข้อมูลหน่วยบริการใหม่</h3>
                        <svg class="text-gray-500 hover:text-gray-900 hover:cursor-pointer transition duration-200 hover:scale-105"
                    ...
                    <div class="grid grid-1 space-y-2 text-sm">
                        <div class="space-y-1">
                            <div class="flex items-center justify-between text-sm">
                                <label for="create_fac_name">ชื่อหน่วยบริการ</label>
                                <input
                                    class="border border-gray-300 rounded p-1 focus:ring-sky-300 focus:ring-3 focus:border-sky-200"
                        ...
                        <div class="space-y-1">
                            <div class="flex items-center justify-between text-sm">
                                <label for="create_fac_code">ชื่อย่อหน่วยบริการ</label>
                                <input
                                    class="border border-gray-300 rounded p-1 focus:ring-sky-300 focus:ring-3 focus:border-sky-200"
                                    type="text" id="create_fac_code" x-model="facultyForm.faculty_code">
                            </div>
        ...
        return {
            ...
            validateForm() {
                ...
                if (!faculty_name || faculty_name.trim() === '') {
                    this.FormErrors.faculty_name = 'กรุณากรอกชื่อหน่วยบริการ';
                }
            ...
            async submitFacultyForm() {
                ...
                try {
                    ...
                    if (confirmed.isConfirmed) {
                        ...
                        if (result.success) {
                            Swal.fire({
                                icon: "success",
                                title: "สำเร็จ",
                                text: `${action}หน่วยบริการเรียบร้อยแล้ว`,
                                timer: 1500,
                    ...
                    await Swal.fire({
                        icon: "error",
                        title: "ไม่สำเร็จ",
                        html: `<p>ไม่สามารถ${action}หน่วยบริการได้</p><p></p> ${error.message}</p><br><hr><p class='text-xs'>หากพบปัญหาในการใช้งาน สามารถติดต่อศูนย์ฯด้วยตนเอง เพื่อดำเนินการแก้ไข</p>`,
                        timer: 5000,
                        showConfirmButton: true,
            ...
            async saveEditFaculty() {
                if (!this.editFacultyForm.faculty_name || this.editFacultyForm.faculty_name.trim() === '') {
                    Swal.fire('แจ้งเตือน', 'กรุณากรอกชื่อหน่วยบริการ', 'warning');
                    return;
                }
                ...
                try {
                    ...
                    Swal.fire({
                        icon: "error",
                        title: "ไม่สำเร็จ",
                        html: `<p>ไม่สามารถแก้ไขหน่วยบริการได้</p><p></p> ${error.message}</p><br><hr><p class='text-xs'>หากพบปัญหาในการใช้งาน สามารถติดต่อศูนย์ฯด้วยตนเอง เพื่อดำเนินการแก้ไข</p>`,
                        timer: 5000,
                        showConfirmButton: true,
                        confirmButtonColor: '#009966',
                        confirmButtonText: "ปิด"
            ...
            confirmDeleteFaculty() {
                ...
                Swal.fire({
                    title: "ยืนยันการลบ?",
                    html: `<p>ต้องการลบหน่วยบริการ "${this.selectedFaculty.faculty_name}" ใช่หรือไม่?</p><br><hr><p class='text-xs text-red-500 mt-1'>กระบวนการเสี่ยงต่อความผิดพลาดของข้อมูล, กรุณาดำเนินการด้วยความระมัดระวัง</p>`,
                    icon: "warning",
                    showCancelButton: true,
