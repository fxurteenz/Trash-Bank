<div x-data="DonationHistoryHandler()" x-init="init()" class="space-y-6">
  ...
  <div class="bg-white rounded-xl shadow-md p-6 card-hover">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
      <div>
        ...
        <input x-model="filterStartDate" @change="applyFilters()" type="date" class="w-full px-4 py-3 border-2 border-slate-300 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition">
      </div>
      <div>
        <label class="block text-sm font-semibold text-slate-700 mb-2">วันที่สิ้นสุด</label>
        <input x-model="filterEndDate" @change="applyFilters()" type="date" class="w-full px-4 py-3 border-2 border-slate-300 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition">
      </div>
      <div>
        <label class="block text-sm font-semibold text-slate-700 mb-2">ค้นหา</label>
        <input x-model="filterSearch" @keydown.enter="applyFilters()" type="text" placeholder="ชื่อผู้บริจาค/สิ่งของ" class="w-full px-4 py-3 border-2 border-slate-300 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition">
      </div>
      <div class="flex gap-2">
        <button @click="applyFilters()" class="flex-1 px-4 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-colors">🔍 ค้นหา</button>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-pink-500 card-hover">
      ...
      <p class="text-3xl font-bold text-pink-600" x-text="donations.reduce((s,x)=>s+Number(x.donation_quantity||0),0)"></p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-orange-500 card-hover">
      ...
      <p class="text-3xl font-bold text-orange-600" x-text="(donations.length>0?(donations.reduce((s,x)=>s+Number(x.donation_item_value||0),0)/donations.length).toFixed(2):0) + ' ฿'"></p>
    </div>
  </div>
        ...
        <tbody class="divide-y divide-slate-200">
          <template x-for="(donation, index) in donations" :key="donation.donation_id">
            <tr class="hover:bg-purple-50 transition cursor-pointer" @click="openDetail(donation)">
              ...
              <td class="px-6 py-4 text-sm text-slate-600" x-text="new Date(donation.created_at).toLocaleDateString('th-TH')"></td>
              <td class="px-6 py-4 text-sm text-slate-900 font-medium" x-text="donation.member_name || 'บริจาค'"></td>
              <td class="px-6 py-4 text-sm text-slate-700" x-text="donation.donation_description || '-'"></td>
              <td class="px-6 py-4 text-sm text-center text-slate-700" x-text="donation.donation_quantity"></td>
              <td class="px-6 py-4 text-sm text-center font-semibold text-purple-600" x-text="Number(donation.donation_item_value||0).toFixed(2)"></td>
              <td class="px-6 py-4 text-center">
                <div class="flex justify-center gap-2">
                  <button @click.stop="openDetail(donation)" class="px-3 py-2 bg-purple-100 text-purple-700 hover:bg-purple-200 rounded-lg font-medium text-sm transition">👁️ ดู</button>
                  <button @click.stop="confirmDelete(donation.donation_id)" class="px-3 py-2 bg-red-100 text-red-700 hover:bg-red-200 rounded-lg font-medium text-sm transition">🗑️ ลบ</button>
                </div>
              </td>
            </tr>
          </template>
          <template x-if="donations.length === 0">
            <tr>
              <td colspan="7" class="px-6 py-8 text-center text-slate-500 font-medium">ไม่มีข้อมูลบริจาค</td>
            </tr>
          </template>
        </tbody>
      </table>
          ...
          <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg">
              <div>
                ...
                <p class="text-base font-bold text-slate-900" x-text="selectedDonation.member_name || 'บริจาค'"></p>
              </div>
              <div>
                <p class="text-sm text-slate-600 font-medium">วันที่บริจาค</p>
                <p class="text-base font-bold text-slate-900" x-text="new Date(selectedDonation.created_at).toLocaleDateString('th-TH')"></p>
              </div>
              <div>
                <p class="text-sm text-slate-600 font-medium">เจ้าหน้าที่รับ</p>
                <p class="text-base font-bold text-slate-900" x-text="selectedDonation.staff_name || '-'"></p>
              </div>
              <div>
                <p class="text-sm text-slate-600 font-medium">หมายเหตุ</p>
                <p class="text-base font-bold text-slate-900" x-text="selectedDonation.donation_description || '-'"></p>
              </div>
            </div>
          </div>
        </template>
                ...
                <tbody class="divide-y divide-slate-200">
                  <template x-for="(item, idx) in detailItems" :key="idx">
                    <tr class="hover:bg-slate-50">
                      ...
                      <td class="px-4 py-3 text-center text-slate-700" x-text="item.donation_quantity"></td>
                      <td class="px-4 py-3 text-right font-semibold text-purple-600" x-text="Number(item.donation_item_value||0).toFixed(2)"></td>
                    </tr>
                  </template>
                </tbody>
              </table>
  ...
  return {
    ...
    async applyFilters() {
      try {
        ...
        if (json.success) {
          this.donations = json.result?.data || [];
        }
      } catch (err) {
