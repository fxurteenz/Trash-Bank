...
<div x-data="WasteSaleHistoryHandler()" x-init="init()" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-orange-500 card-hover">
            ...
            <p class="text-3xl font-bold text-orange-600"
                x-text="(sales.length>0?(sales.reduce((s,x)=>s+Number(x.waste_sale_total_price||0),0)/sales.length).toFixed(2):0) + ' ฿'">
            </p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 card-hover">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
            ...
            <div>
                ...
                <input x-model="filterBuyer" @keydown.enter="applyFilters()" type="text" placeholder="ค้นหาผู้ซื้อ"
                    class="w-full px-4 py-3 border-2 border-slate-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition">
            </div>
                ...
                <tbody class="divide-y divide-slate-200">
                    <template x-for="(sale, index) in sales" :key="sale.waste_sale_id">
                        <tr class="hover:bg-slate-50 transition text-sm text-slate-700"
                            @click="openDetail(sale.waste_sale_id)" style="cursor: pointer;">
                            <td class="px-6 py-4 font-medium text-slate-400" x-text="index + 1"></td>
                            <td class="px-6 py-4" x-text="new Date(sale.waste_sale_date).toLocaleDateString('th-TH')">
                            </td>
                            <td class="px-6 py-4 font-medium" x-text="sale.waste_sale_buyer || 'ไม่ระบุ'"></td>
                            <td class="px-6 py-4 text-right font-medium"
                                x-text="Number(sale.waste_sale_total_weight||0).toFixed(2)"></td>
                            <td class="px-6 py-4 text-right font-bold text-emerald-600"
                                x-text="Number(sale.waste_sale_total_price||0).toFixed(2)"></td>
                            <td class="px-6 py-4 text-center">
                                <button @click.stop="openDetail(sale.waste_sale_id)"
                                    class="text-blue-600 hover:text-blue-700 font-semibold mr-3">👁️ ดู</button>
                                <button @click.stop="confirmDelete(sale.waste_sale_id)"
    ...
    <div class="flex justify-center gap-4 pb-8">
        <button @click="printReport()"
            class="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold transition-colors flex items-center gap-2"><span>🖨️</span>
            พิมพ์รายงาน</button>
        <button @click="exportCSV()"
            class="px-8 py-4 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition-colors flex items-center gap-2"><span>📥</span>
            ส่งออก CSV</button>
    </div>
    ...
    <div x-show="showModal" class="fixed inset-0 bg-black/40 flex items-center justify-center p-4"
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl">
            ...
            <div class="p-4 space-y-3" x-show="detail">
                <div class="text-sm text-slate-600">วันที่: <span
                        x-text="new Date(detail.created_at).toLocaleDateString('th-TH')"></span></div>
                <div class="text-sm text-slate-600">ผู้ซื้อ: <span x-text="detail.waste_sale_buyer || '-' "></span>
                </div>
                <div class="text-sm text-slate-600">รวม: <span
                        x-text="Number(detail.waste_sale_total_weight||0).toFixed(2)+' กก. / '+Number(detail.waste_sale_total_price||0).toFixed(2)+' ฿'"></span>
                </div>
                <div class="overflow-x-auto">
                            ...
                            <template x-for="item in detail.details || []" :key="item.waste_sale_detail_id">
                                <tr class="border-b">
                                    ...
                                    <td class="px-3 py-2 text-right"
                                        x-text="Number(item.waste_sale_detail_weight||0).toFixed(2)"></td>
                                    <td class="px-3 py-2 text-right"
                                        x-text="Number(item.waste_sale_detail_price||0).toFixed(2)"></td>
                                    <td class="px-3 py-2 text-right"
                                        x-text="Number(item.waste_sale_detail_price||0).toFixed(2)"></td>
                                </tr>
                            </template>
