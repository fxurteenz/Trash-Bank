                    ...
                    <table class="w-full border-collapse">
                        ...
                        <tbody>
                            <template x-for="(clearance, index) in clearances" :key="clearance.waste_clearance_id">
                                <tr class="border-b border-gray-200 hover:bg-gray-50" @click="showDetail(clearance)" style="cursor: pointer;">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <button @click.stop="deleteRecord(clearance.waste_clearance_id)" class="text-red-600 hover:text-red-700 font-semibold">
                                            🗑️ ลบ
                                        </button>
                                    </td>
                                </tr>
    ...
    return {
        ...
        async deleteRecord(clearanceId) {
            if (confirm('คุณแน่ใจหรือว่าต้องการลบรายการนี้?')) {
                try {
                    const response = await fetch(`/api/clearances/delete/${clearanceId}`, { method: 'POST' });
                    if (response.ok) {
                        alert('ลบรายการสำเร็จ');
                        this.fetchClearances(1);
                    }
                } catch (error) {
