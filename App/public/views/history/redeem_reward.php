                    ...
                    <table class="w-full border-collapse">
                        ...
                        <tbody>
                            <template x-for="(redeem, index) in redeems" :key="redeem.member_reward_id">
                                <tr class="border-b border-gray-200 hover:bg-gray-50" @click="showDetail(redeem)" style="cursor: pointer;">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <button @click.stop="deleteRecord(redeem.member_reward_id)" class="text-red-600 hover:text-red-700 font-semibold">
                                            🗑️ ลบ
                                        </button>
                                    </td>
                                </tr>
...
function RedeemRewardHistoryData() {
    return {
        ...
        showDetail(redeem) {
            alert(`รายละเอียด: ${redeem.reward_name}\nแต้ม: ${redeem.member_reward_point}\nสถานะ: ${redeem.member_reward_status}`);
        },

        async deleteRecord(redeemId) {
            if (confirm('คุณแน่ใจหรือว่าต้องการลบรายการนี้?')) {
                try {
                    const response = await fetch(`/api/member_rewards/delete`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ id: redeemId })
                    });
                    if (response.ok) {
